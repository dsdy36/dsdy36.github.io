...
python convert_for_v1.py submissions_imusic_v5 ../FS-generator/submission
...
| 6 | CW label 模糊 ("deliberately implement incorrectly") | pattern_matrix AI fallback + 硬编码全部改为 Type B 描述 | CW-generator/pattern_matrix.py |
| 7 | CW label AI fallback 含未转义单引号 | `fix_vague_labels.py` 的 `apply_rewrites` 加 `replace("'", "\\'")` | CW-generator/fix_vague_labels.py |
...
| 10 | CW API 调用无缓存 | `main_v5.py` 加 `variant_cache.json` 路径 | CW-generator/main_v5.py |
| 11 | 负向 FS 匹配 reference 被直接删除 | 改为降权 weight=0.5（对齐旧系统策略） | taffies_fs_generator.py |
| 12 | `main_v5.py` 无 Python 输出缓冲 | 加 `-u` flag | 运行命令 |
| 13 | pattern_matrix 硬编码 6 处 Type A check_regex (`(?!...)`) | 全部改为 Type B（检测具体存在的代码模式） | CW-generator/pattern_matrix.py |
| 14 | pattern_matrix 好模式错标为坏（INSERT OR IGNORE 被标为 bad） | 保留但标注为 alternative approach | CW-generator/pattern_matrix.py |
...
- DeepSeek API: `.env` 在 `FS-generator/` 和 `CW-generator/`
...
```
Step 1: CW 确定性注入器
  - 新建 CW-generator/pattern_injector.py             2h
...
## 2026-07-14 审核发现与设计澄清

### 目录名与实际不符

CLAUDE.md 目录树使用 `FS-generator/` 和 `CW-generator/`，实际文件系统为 `FS-generator/` 和 `CW-generator/`。

### CLAUDE.md 列出但实际不存在的文件

`find_example.py`、`verify_readmes.py`、`check_context_patterns.py` 在 CLAUDE.md 目录树中被列出，但文件系统中不存在。`submission/` 和 `output/` 目录当前也为空。

### README 覆盖验证结果

- **46/50** 学生有 README.md（S034-S037 缺失）
- 所有 46 个 README **全部覆盖 14 个 criterion**（每个都有 `### RQ{x}_{y}:` 节）
- 每个学生 1-2 个 Error pattern + 12-13 个 Correct approach（正好 14 节）
- 即：**每个 criterion 在每份 README 中都有 label**，但不一定是 Error label——大多数 criterion 标注的是"✅ Correct approach"

### CW 数据分布分析（per-criterion error 覆盖）

各 criterion 的 error 学生数（46 人中）：

```
RQ1_1: 12   RQ1_2:  9   RQ1_3: 12   RQ1_4: 10
RQ2_1: 10   RQ2_2: 10   RQ2_3: 10   RQ2_4: 12
RQ3_1: 15   RQ3_2: 15   RQ3_3: 10   RQ3_4: 13
RQ3_5: 11   RQ3_6:  9
```

分布 9-15 个/criterion，占 20-33%，看起来合理。但存在 **poor tier 通胀问题**：

**Poor tier 效应**：S045-S050（6 个 poor tier 学生）全 criterion 注入错误，给每个 criterion 贡献恰好 6 个错误。以 RQ3_6 为例：9 个 error 中 6 个来自 poor tier（67%），只有 3 个（7.5%）是正常分布中的真实错误。

| Criterion | 总 Error | Poor tier 贡献 | 非 Poor tier 实际 |
|-----------|:--:|:--:|:--:|
| RQ1_1 | 12 | 6 (50%) | 6 (15%) |
| RQ3_6 | 9 | 6 (67%) | 3 (7.5%) |
| RQ3_1 | 15 | 6 (40%) | 9 (22.5%) |

**对 FS 生成的影响**：
1. Poor tier 学生代码是"全盘崩溃"——所有函数都写错。FS 可能学到检测"代码整体崩坏"而非该 criterion 的特定错误
2. 错误多样性不足——6 个 poor tier 学生的同一 criterion 错误可能来自同一 AI 生成模板，实际只有 1-2 种错误形态
3. 正确代码多样性也受限——AI 生成的"正确代码"风格高度一致，FS 学到的"正确边界"过窄

**CW 数据生成 FS 对真实评分的风险**：

| 风险 | 原因 | 表现 |
|------|------|------|
| **漏报** | FS 只见过 pattern_matrix 定义的错误形态 | 真实学生"没见过"的创造性错误被漏掉 |
| **误报** | FS 习惯 AI 的干净正确代码 | 真实学生奇怪但正确的写法被误判 |
| **分布偏移** | CW 数据和真实学生代码分布不同 | 整体 P/R 在真实场景下可能显著下降 |

### 学生函数提取机制（已处理 def stub 但硬编码需改进）

当前代码已处理"给定 def stub 下完成函数体"的场景：

```
STUDENT_FUNCTIONS (frozenset) → 硬编码学生函数名
_extract_student_functions()    → 用正则匹配 def func_name(...): 提取函数体
CRIT_FUNC (dict)               → criterion → 相关函数名的硬编码映射
```

CRIT_FUNC 当前值（iMusic 专用，`taffies_fs_generator.py:466`）：
...
**泛用性问题**：换一个 Python 题目，需手动改 CRIT_FUNC 和 STUDENT_FUNCTIONS。改进方案：从 rubric JSON 读取 `applies_to_functions` 字段，从 question template 用 AST 解析区分 template 函数和 student 函数。

### `_scoring_weight` 语义澄清

**不是学生评分权重，是 FS 内部质量门控标记**（字段 `_` 前缀暗示内部使用）。

| 值 | 含义 | 触发条件 |
|:--:|------|------|
| 1.0 (默认) | 正常使用 | 新生成的 FS、literal fallback FS |
| 0.5 | 保留但降权 | 负向 FS 匹配 reference、正向 FS 匹配 template、正负冲突 |
| 0.0 | 排除/停用 | Broad Filter（>40%）、去重重复、criterion 无 bad pattern、匹配零学生 |

所有下游模块（blind_test、round5_refine、coverage、analyze_fp）用 `== 0.0` 做筛选——权重为零直接跳过。

### 现状总结.txt 与代码的 4 处不符

以下 4 处描述与 `plan_d.py` 实际代码矛盾：

| # | 文档说法 | 实际代码 | 行号 |
|:--:|------|------|:--:|
| 1 | MATCHES_NOTHING Gate "在 1962-1970 行被注释掉" | **活跃运行**，支持 check_function + regex | 2226-2300 |
| 2 | "不存在任何 re.escape() 字面模式生成逻辑" | `_apply_literal_fallback()` **已实现形态 A 兜底** | 1755-1916 |
| 3 | Broad Filter "只检查 regex，不处理 check_function" | **同时处理两种类型** | 1498-1520 |
| 4 | Dedup "功能没有被替代" | `_deduplicate_fs()` **已实现且支持两种类型** | 1415-1460 |

### cluster_by_pattern 改进方案（5 项）

当前实现位于 `taffies_fs_generator.py:527`。

**问题诊断**：

| # | 问题 | 泛用性影响 |
|:--:|------|:--:|
| 1 | `_normalize_label` 纯字符串匹配——语义相同但措辞不同的 label 分到不同组 | ⚠️ 换题目/CW 版本会翻车 |
| 2 | 单维度分组——只看 pattern label，不看 quality tier | ✅ 阻��质量分层 |
| 3 | Representative 选字母序第一个——不是代码中位数 | ⚠️ 影响 FS 代表性 |
| 4 | Good/Bad 分开聚类——同一 criterion 好坏模式不联动 | ⚠️ |
| 5 | `STUDENT_FUNCTIONS` + `CRIT_FUNC` 硬编码 | ✅ 致命 |

**改进方案**：

1. **语义 label 聚类**：CW 场景用 pattern_id 做 key（确定性），真实学生回退 LLM 语义判断
2. **多维度分组**：key = `(pattern_label, quality_tier)`，让 AI 看到同 label 不同质量的代码对比
3. **Representative 改为代码中位数**：挑 token 重叠度最高的学生，而非字母序第一
4. **Good/Bad 联合分组**：一次调用同时返回 bad_clusters、good_clusters、mixed_students
5. **Rubric 驱动 criterion→function 映射**：从 rubric JSON 读 `applies_to_functions`，替代 CRIT_FUNC 硬编码。question template 用 AST 自动区分 template 函数 vs student 函数。

### 多质量分层设计（MD 分组 × Quality Tier）

CW-generator 在生成时预标注每个 criterion 的 quality tier（per-criterion，不是 per-student）。FS 端分组时 key = `(pattern_label, quality_tier)`。AI 同时看到：
- 同一 pattern label 下不同 quality tier 的代码对比
- 该 criterion 的 rubric 原文
- Good patterns 和 bad patterns 的关联

目标是生成能区分质量层次的 FS，而非二元 VULNERABLE/SAFE 判定。

### 沙箱接口边界

**管线分离方案**（结论：明显优于嵌入 check function）：

```
FS 端管线                       沙箱团队管线
──────────                      ────────────
语法特征提取                    跑沙箱 → VULNERABLE/SAFE 标签
    │                              │
    ├── 语法可判的 criterion       │
    │   → 直接产出 check function  │
    │                              │
    └── 语法不可判的 criterion     │
        ...
        └── 语法特征 + 沙箱标签 → AI 生成 FS
            → 找到区分特征 → 产出语法 FS
            → 找不到 → 标记 "behavioral-only"
```

关键：沙箱标签是**训练信号**（告诉 AI 谁对谁错），不是 check function 的运行时依赖。语法不可判不一定是全部学生代码都无法判断——可能部分学生语法特征清晰，部分需要沙箱。

### README 格式作为标准

未来所有 Python 题目的 README 需标准化为：

```markdown
### RQ{x}_{y}:

**❌ Error pattern**
- [A] description of the error pattern variant
- [B] another variant

**✅ Correct approach**
- [A] description of the correct pattern variant
```

此格式已由 `cluster_by_pattern()` 解析，赋值无关（assignment-agnostic）。

### plan_d.py 中已实现但现状总结未记录的 Gate

以下功能实际存在且活跃，文档描述为"缺失"或"不生效"：

| 功能 | 位置 | 状态 |
|------|:--:|:--:|
| Literal Fallback（形态 A 兜底） | plan_d.py:1755 | ✅ 活跃 |
| Dedup（FS 去重） | plan_d.py:1415 | ✅ 活跃 |
| MATCHES_NOTHING Gate | plan_d.py:2226 | ✅ 活跃 |
| Broad Filter（check_function 支持） | plan_d.py:1498 | ✅ 活跃 |
