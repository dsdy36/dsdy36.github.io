...
def _deduplicate_fs(all_fs: list[dict]):
    """Merge FS with near-identical check functions or regex patterns.

    Normalizes whitespace, wildcard, and common variable differences
    to find semantic duplicates. Keeps the one with the most descriptive name
    and marks others as duplicates (weight=0.0).
    Supports both check_function and regex signature types.
    """
    from collections import defaultdict
    ...
    for fs in all_fs:
        ...
        if sig_type == 'check_function':
            ...
            # Normalize: strip whitespace, collapse blank lines
            norm = re.sub(r'\s+', ' ', fn_body.strip())
            # Normalize variable names in string literals (they vary between generations)
            norm = re.sub(r'["\'][^"\']*?["\']', '"..."', norm)
            ...
            if not regex:
                continue
            norm = re.sub(r'\\s\+|\\s\*|\\s\{1,\}|\\s\?', r'\\s*', regex)
    ...
    for _, group in dup_groups.items():
        if len(group) < 2:
            continue
        # Keep the one with the longest name (most descriptive), mark others
        group.sort(key=lambda f: len(f.get('name', '')), reverse=True)
        keeper = group[0]
        for dup in group[1:]:
            dup['_duplicate_of'] = keeper.get('id', '?')
            dup['_scoring_weight'] = 0.0
            dup_count += 1
            print(f'  DEDUP: {dup.get("id","?")} -> duplicate of {keeper.get("id","?")} '
                  f'[{dup.get("criterion","?")}]')

    print(f'  Deduplicated: {dup_count} FS merged')
...
def _apply_broad_negative_filter(all_fs: list[dict],
    ...
    for fs in all_fs:
        ...
        # Count matches — support both check_function and regex
        matched = 0
        sig_type = fs.get('signature_type', 'regex')
        if sig_type == 'check_function':
            fn_body = fs.get('check_function', '').strip()
            if fn_body:
                try:
                    import ast as _ast
                    _ast.parse(fn_body)
                    local_ns = {}
                    exec(fn_body, {'__builtins__': __builtins__}, local_ns)
                    check_fn = local_ns.get('check')
                    if callable(check_fn):
                        for c in codes:
                            try:
                                result = check_fn(c)
                                ok = result[0] if isinstance(result, tuple) and len(result) == 2 else result
                                if ok:
                                    matched += 1
                            except Exception:
                                pass
...
# ── Criterion metadata: which criteria are marking-critical ──
# Default: all RQ criteria are marking-critical (contribute to coursework grade).
# Override per-criterion if some are informational-only.
_MARKING_CRITICAL_DEFAULT = True  # all criteria are marking-critical by default
...
def _apply_literal_fallback(
    ...
    all_readmes: dict,
    task_subs: dict[int, list[dict]],
    all_rubric_criteria: list[dict],
):
    """Generate literal (re.escape) FS for marking-critical criteria with zero coverage.

    After all semantic FS have passed through quality gates, some marking-critical
    criteria may have zero active negative FS (all were removed or none were generated).
    For these criteria, we fall back to literal mechanical matching: extract key code
    lines from gap students, escape them, and create simple check functions.

    This implements the Type A/Type B hybrid: Type B (semantic) is preferred, but
    Type A (literal) is the safety net for criteria that MUST be scored.
    """
    print(f'\n{"=" * 60}')
    print('  LITERAL FALLBACK — Marking-Critical Coverage Guarantee')
    print('  (generating re.escape FS for criteria with 0 active neg FS)')
    print('=' * 60)

    # Build criterion -> task mapping from rubric
    crit_task = {}
    crit_is_marking = {}
    for rc in all_rubric_criteria:
        rid = rc.get('id', '')
        if not rid:
            continue
        m = re.search(r'(\d)', rid)
        tn = int(m.group(1)) if m else 1
        crit_task[rid] = f'Task{tn}'
        crit_is_marking[rid] = rc.get('marking_critical', _MARKING_CRITICAL_DEFAULT)

    # Find which criteria have bad patterns (should have negative FS)
    criteria_with_bad = set()
    for sid, rdata in all_readmes.items():
        criteria = rdata.get('criteria', {})
        if isinstance(criteria, str):
            try:
        ...
        for crit, patterns in criteria.items():
            if patterns.get('bad', []) or patterns.get('mistake', []):
                criteria_with_bad.add(crit)

    # Per-criterion: count active negative FS
    crit_neg_count = defaultdict(int)
    for fs in all_fs:
        if fs.get('fs_type') == 'negative' and fs.get('_scoring_weight', 1.0) > 0:
            crit_neg_count[fs.get('criterion', '')] += 1

    # Find marking-critical criteria with bad patterns but zero active neg FS
    fallen_back = 0
    for crit in sorted(criteria_with_bad):
        if not crit_is_marking.get(crit, True):
            continue  # informational-only criterion — coverage not required
        if crit_neg_count.get(crit, 0) > 0:
            continue  # already covered

        task_str = crit_task.get(crit, 'Task1')
        tn = int(task_str.replace('Task', '')) if task_str.startswith('Task') else 1

        print(f'\n  [{crit}] MARKING-CRITICAL with 0 active negative FS — generating literal fallback')

        # Collect gap students for this criterion
        gap_students = []
        for sid, rdata in all_readmes.items():
            criteria = rdata.get('criteria', {})
            if isinstance(criteria, str):
            ...
            if not isinstance(criteria, dict):
                continue
            crit_data = criteria.get(crit, {})
            if crit_data.get('bad', []) or crit_data.get('mistake', []):
                gap_students.append(sid)

        # For each gap student, extract key lines and create literal FS
        fs_id_counter = 0
        for sid in gap_students[:5]:  # max 5 literal FS per criterion
            # Get student code
            code = ''
            for s in task_subs.get(tn, []):
                if s['student'] == sid:
            ...
            from taffies_fs_generator import _extract_student_functions
            extracted = _extract_student_functions(code, crit)
            if not extracted or len(extracted.strip()) < 30:
                extracted = code

            # Get key lines: non-empty, non-comment, non-import lines
            key_lines = []
            ...
            # Take up to 3 most distinctive lines (longest = most specific)
            key_lines.sort(key=len, reverse=True)
            key_lines = key_lines[:3]

            # Build a literal check function using re.escape
            escaped_patterns = [re.escape(line) for line in key_lines]
            pattern_list = ', '.join(f'r"{ep}"' for ep in escaped_patterns[:2])
            ...
            check_fn = (
                ...
                '            return (True, f"matched: {pat[:60]}")\n'
                '    return (False, "")\n'
            )

            fs_id_counter += 1
            literal_fs = {
                'id': f'FS_LITERAL_{crit}_{fs_id_counter}',
                'name': f'[LITERAL FALLBACK] {crit} — {sid}',
                'fs_type': 'negative',
                'criterion': crit,
                'check_function': check_fn,
                'signature_type': 'check_function',
                'regex_flags': 'IGNORECASE',
                'feedback': (
                    f'Your code in this criterion may contain an issue. '
                    f'Please review the rubric requirement and check your implementation.'
                ),
                'marks': -1,
                'evidence': f'literal fallback for {sid}',
                'task': task_str,
                'files': [f'task{tn}.py'],
                'auto_generated': True,
                'source': 'literal_fallback',
                'source_detail': f'gap student {sid}, {len(key_lines)} key lines',
                '_scoring_weight': 1.0,
                '_literal_fallback': True,
            }
            all_fs.append(literal_fs)
            fallen_back += 1
            print(f'    +1 literal FS for {sid}: {len(key_lines)} key lines, '
                  f'patterns: {key_lines[0][:50]}...')

    if fallen_back == 0:
        print('  No marking-critical criteria need literal fallback — all covered.')
    else:
        print(f'\n  Literal fallback: +{fallen_back} FS across marking-critical criteria')
    print(f'  Total FS after fallback: {len(all_fs)}')
...
def run_plan_d(question_dir: str, submissions_dir: str,
    ...
    # ── Post-processing: Deduplicate FS ──
    # Merges FS with near-identical check functions or regex patterns.
    # Normalizes whitespace/wildcard differences to find semantic duplicates.
    print(f'\n{"=" * 60}')
    print('  DEDUPLICATE FS')
    print('=' * 60)
    _deduplicate_fs(all_fs)
    _save_plan_d_snapshot(all_fs, question_id, '04c_post_dedup')
    ...
    # (dedup and variable gen are now handled by dedicated steps above)

    # ── Literal Fallback for Marking-Critical Criteria ──
    # After all quality gates have run, check each marking-critical criterion.
    # If it has zero active negative FS, generate re.escape() literal FS
    # from gap students' key code lines to guarantee coverage.
    # This implements the "Type A fallback" — literal mechanical matching
    # as a safety net when semantic FS generation fails.
    _apply_literal_fallback(all_fs, all_readmes, task_subs, all_rubric_criteria)
    _save_plan_d_snapshot(all_fs, question_id, '07b_post_literal_fallback')

    # ── Late-Stage MATCHES_NOTHING Gate ──
    # Runs on full student sample to catch FS that compile but match nobody.
    # Supports both check_function and regex signature types.
    print(f'\n{"=" * 60}')
    print('  LATE-STAGE GATE: MATCHES_NOTHING')
    print('  (checking all FS against full student sample)')
    print('=' * 60)
    BATCH_TASK_MAP = {'q1-': 'Task1', 'q2-': 'Task2', 'q3-': 'Task3'}

    matches_nothing_removed = 0
    for fs in all_fs:
        fid = fs.get('id', '?')
        fs_task = fs.get('task', '')
        if not fs_task:
        ...
        # Collect all student codes for this FS's task
        task_codes_list = []
        for tn in [1, 2, 3]:
            if f'Task{tn}' == fs_task:
                task_codes_list = [s.get('code', '') for s in task_subs.get(tn, [])
                                   if s.get('code', '').strip()]
                break

        if not task_codes_list:
            continue

        # Check if FS matches ANY student
        matched_any = False
        sig_type = fs.get('signature_type', 'regex')
        ...
        if not matched_any:
            ...
            fs['_scoring_weight'] = 0.0
            matches_nothing_removed += 1
            print(f'  EXCLUDED {fid}: matched 0/{len(task_codes_list)} students '
                  f'— {fs.get("name","?")[:50]}')

    print(f'  MATCHES_NOTHING excluded: {matches_nothing_removed}')
    _save_plan_d_snapshot(all_fs, question_id, '07_post_late_gate')
