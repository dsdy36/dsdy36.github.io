...
def _normalize_label(label: str) -> str:
    """Normalize a pattern label for clustering key generation.

    Strips variant prefixes like [A] [B], normalizes whitespace and case,
    and canonicalizes common spelling variants.
    """
...
def _build_crit_func_from_rubric(rubric_path: str = None) -> dict[str, list[str]]:
    """Build CRIT_FUNC mapping dynamically from rubric_cache.json.

    Reads target_functions per task from rubric cache and maps each criterion
    to its relevant student functions. This replaces the hardcoded CRIT_FUNC
    dict for assignment-agnostic operation.

    Args:
        rubric_path: Path to rubric_cache.json. Auto-detected if None.

    Returns:
        {criterion_id: [function_names]} mapping.
    """
    if rubric_path is None:
        rubric_path = os.path.join(BASE_DIR, 'output', 'q1_iMusic', 'rubric_cache.json')

    if not os.path.exists(rubric_path):
        print(f'  [WARN] rubric_cache.json not found, using hardcoded CRIT_FUNC fallback')
        return dict(CRIT_FUNC)

    try:
        with open(rubric_path, 'r', encoding='utf-8') as f:
            rubric = json.load(f)
    except Exception:
        print(f'  [WARN] Failed to parse rubric_cache.json, using hardcoded CRIT_FUNC fallback')
        return dict(CRIT_FUNC)

    crit_to_task: dict[str, str] = {}
    task_to_funcs: dict[str, list[str]] = {}

    for task in rubric.get('tasks', []):
        task_id = task.get('id', '')
        target_funcs = task.get('target_functions', [])
        if task_id and target_funcs:
            task_to_funcs[task_id] = target_funcs

        for rc in task.get('rubric_criteria', []):
            crit_id = rc.get('id', '')
            if crit_id and task_id:
                crit_to_task[crit_id] = task_id

    # Validate target_functions against actual function definitions in template
    # (rubric may include non-function names like Flask route names)
    template_path = os.path.join(BASE_DIR, 'question', 'code', 'iMusic.py')
    actual_func_names: set[str] = set()
    if os.path.exists(template_path):
        with open(template_path, 'r', encoding='utf-8') as f:
            actual_func_names = set(re.findall(r'^def\s+(\w+)\s*\(', f.read(), re.MULTILINE))

    crit_func: dict[str, list[str]] = {}
    for crit_id, task_id in crit_to_task.items():
        funcs = task_to_funcs.get(task_id, [])
        # Filter to only actual function names
        if actual_func_names:
            funcs = [f for f in funcs if f in actual_func_names]
        if funcs:
            crit_func[crit_id] = funcs
        else:
            # Fallback for this criterion: use hardcoded value if available
            crit_func[crit_id] = CRIT_FUNC.get(crit_id, [])

    # Merge any criteria from hardcoded CRIT_FUNC not found in rubric
    for crit_id, funcs in CRIT_FUNC.items():
        if crit_id not in crit_func:
            crit_func[crit_id] = funcs

    print(f'  [INFO] Built CRIT_FUNC from rubric: {len(crit_func)} criteria mapped '
          f'(rubric: {len(crit_to_task)}, hardcoded fallback: {len(CRIT_FUNC) - len(crit_to_task)})')
    return crit_func


def _detect_student_functions_from_template(
    template_path: str = None,
    exclude_names: set[str] = None,
) -> set[str]:
    """Detect student-written functions by parsing the question template with AST.

    Student stubs are functions whose body is just 'pass', '...', a docstring,
    or a trivial return. Template-provided functions have substantive bodies.

    This replaces the hardcoded STUDENT_FUNCTIONS frozenset for assignment-agnostic
    operation.

    Args:
        template_path: Path to question template (e.g. iMusic.py). Auto-detected.
        exclude_names: Additional function names to exclude (e.g. Flask routes).

    Returns:
        Set of function names that are student-completion stubs.
    """
    if template_path is None:
        template_path = os.path.join(BASE_DIR, 'question', 'code', 'iMusic.py')

    if exclude_names is None:
        exclude_names = {'upload_route', 'index', 'main', 'page_not_found'}

    if not os.path.exists(template_path):
        print(f'  [WARN] Template not found, using hardcoded STUDENT_FUNCTIONS fallback')
        return set(STUDENT_FUNCTIONS)
    ...
    try:
        ...
        print(f'  [WARN] Failed to parse template ({e}), using hardcoded fallback')
        return set(STUDENT_FUNCTIONS)

    student_funcs: set[str] = set()
    for node in _ast.walk(tree):
        if isinstance(node, _ast.FunctionDef):
            # Skip Flask route functions (template-provided)
            has_flask_route = False
            for decorator in node.decorator_list:
                if isinstance(decorator, _ast.Call):
                    func = decorator.func
                    if isinstance(func, _ast.Attribute) and func.attr == 'route':
                        has_flask_route = True
                        break
            ...
            if len(body) == 0:
                ...
                if isinstance(stmt, _ast.Pass):
                    is_stub = True
                elif isinstance(stmt, _ast.Expr) and isinstance(stmt.value, _ast.Constant):
                    if stmt.value.value is Ellipsis or stmt.value.value is None:
                        is_stub = True
                elif isinstance(stmt, _ast.Return) and (
                    stmt.value is None or
                    (isinstance(stmt.value, _ast.Constant) and stmt.value.value is None)
                ):
                    is_stub = True
            elif len(body) <= 3:
                # Heuristic: short function bodies that are mostly pass/docstring
                non_trivial = [s for s in body
                              if not isinstance(s, (_ast.Pass, _ast.Expr))]
                if len(non_trivial) == 0:
                    is_stub = True

            if is_stub and node.name not in exclude_names and not node.name.startswith('_'):
                student_funcs.add(node.name)

    if student_funcs:
        print(f'  [INFO] Auto-detected {len(student_funcs)} student stub functions: '
              f'{sorted(student_funcs)}')
        return student_funcs
    else:
        print(f'  [WARN] No student stubs detected in template, using hardcoded fallback')
        return set(STUDENT_FUNCTIONS)


def _get_resolved_crit_func(rubric_path: str = None, prefer_hardcoded: bool = True) -> dict[str, list[str]]:
    """Get CRIT_FUNC, merging hardcoded and rubric-driven mappings.

    By default, prefers the precise hardcoded mapping (one function per criterion)
    and falls back to rubric for unknown criteria. This ensures:
    - iMusic and known assignments use the tested, precise mappings
    - New assignments automatically work via rubric without code changes

    Set prefer_hardcoded=False to use rubric data exclusively.

    Caches the result so rubric is only loaded once.
    """
    if not hasattr(_get_resolved_crit_func, '_cache'):
        rubric_driven = _build_crit_func_from_rubric(rubric_path)
        merged = {}

        if prefer_hardcoded:
            # Start with rubric (covers all criteria for new assignments)
            merged.update(rubric_driven)
            # Override with hardcoded for precision on known criteria
            merged.update(CRIT_FUNC)
        else:
            merged = rubric_driven

        _get_resolved_crit_func._cache = merged

    return _get_resolved_crit_func._cache


def _get_resolved_student_functions(template_path: str = None) -> set[str]:
    """Get STUDENT_FUNCTIONS, preferring template auto-detection over hardcoded.

    Caches the result so template is only parsed once.
    """
    if not hasattr(_get_resolved_student_functions, '_cache'):
        detected = _detect_student_functions_from_template(template_path)
        _get_resolved_student_functions._cache = detected
    return _get_resolved_student_functions._cache


# Legacy hardcoded — kept as fallback.
...
# Legacy hardcoded CRIT_FUNC — kept as fallback.
...
def _extract_student_functions(code: str, criterion: str = '',
                               use_rubric: bool = False,
                               rubric_path: str = None) -> str:
    ...
    When use_rubric=True, uses rubric-driven CRIT_FUNC (assignment-agnostic).
    """
    if use_rubric:
        crit_func = _get_resolved_crit_func(rubric_path)
        student_funcs = _get_resolved_student_functions()
    else:
        crit_func = CRIT_FUNC
        student_funcs = STUDENT_FUNCTIONS

    target = crit_func.get(criterion, []) if criterion else list(student_funcs)
    if not target:
        target = list(student_funcs)
...
def _pick_representative(sids: list[str], sub_lookup: dict[str, str],
                        criterion: str = '', method: str = 'centrality') -> tuple[str | None, str]:
    """Pick most representative student for a cluster.

    Args:
        sids: Student IDs in this cluster.
        sub_lookup: {student_id: code} mapping.
        criterion: Optional criterion ID for function extraction.
        method: 'centrality' — pick the student whose code token set has highest
                average Jaccard similarity with all other students in the group
                (the "code median").
                'first' — original behavior (alphabetical first with valid code).

    Returns:
        (student_id, extracted_code) or (None, '') if no valid student found.
    """
    if method == 'first' or len(sids) <= 2:
        # Original alphabetical-first behavior
    ...
    # ── Centrality method: token overlap with group median ──
    import tokenize as _tokenize, io as _io

    token_sets: dict[str, tuple[str, set[str]]] = {}
    valid_sids: list[str] = []
    ...
    for sid in sorted(sids):
        ...
        try:
            tokens = {tok.string for tok in _tokenize.generate_tokens(
                _io.StringIO(extracted).readline) if tok.type == _tokenize.NAME}
            token_sets[sid] = (extracted, tokens)
            valid_sids.append(sid)
        except Exception:
            continue

    if len(valid_sids) < 2:
        return _pick_representative(sids, sub_lookup, criterion, method='first')

    # Pairwise Jaccard similarity → pick highest average similarity
    best_sid = valid_sids[0]
    best_score = -1.0
    for sid_a in valid_sids:
        tokens_a = token_sets[sid_a][1]
        if not tokens_a:
            continue
        total_sim = 0.0
        for sid_b in valid_sids:
            if sid_a == sid_b:
                continue
            tokens_b = token_sets[sid_b][1]
            if not tokens_b:
                continue
            intersection = len(tokens_a & tokens_b)
            union = len(tokens_a | tokens_b)
            total_sim += intersection / union if union > 0 else 0.0
        avg_sim = total_sim / (len(valid_sids) - 1)
        if avg_sim > best_score:
            best_score = avg_sim
            best_sid = sid_a

    return best_sid, token_sets[best_sid][0]
...
def cluster_by_pattern(
    ...
    use_quality_tiers: bool = False,
) -> list[dict]:
    """Cluster students by their specific good or bad pattern for a criterion.

    Groups students by (normalized_pattern_label, quality_tier) tuple.
    This ensures AI sees code of similar quality together, enabling multi-tier
    check function generation.

    Args:
        all_readmes: {student_id: readme_data} mapping.
        task_subs: {task_num: [{student, code, ...}]} mapping.
        criterion: e.g. 'RQ2_3'.
        pattern_type: 'good', 'bad', or 'mistake'.
        min_cluster_size: Only return clusters with >= this many students.
        use_quality_tiers: If True, include quality_tier in grouping key.

    Returns:
        [{label, students, count, representative_student, representative_code,
          quality_tier, pattern_type}, ...]
    """
    task_num = int(re.search(r'(\d)', criterion).group(1))
    subs = task_subs.get(task_num, [])
    sub_lookup = {s['student']: s.get('code', '') for s in subs}

    # Key: (normalized_label, quality_tier) or just normalized_label
    ...
    for sid, rdata in all_readmes.items():
        ...
        quality_tier = rdata.get('quality_tier', 'unknown') if use_quality_tiers else 'unknown'

        for desc in pattern_list:
            if desc.strip():
                label_key = _normalize_label(desc.strip())
                if use_quality_tiers:
                    key = (label_key, quality_tier)
                else:
                    key = (label_key,)
    ...
    clusters = []
    skipped = 0
    for key, sids in sorted(pattern_groups.items(), key=lambda x: -len(x[1])):
        if len(sids) < min_cluster_size:
            skipped += 1
            continue

        # Unpack grouping key
        if use_quality_tiers and isinstance(key, tuple) and len(key) == 2:
            label, quality_tier = key
    ...
    if skipped:
        quality_msg = ' (consider lowering min_cluster_size)' if use_quality_tiers else ''
        print(f'    ({skipped} clusters < {min_cluster_size} students '
              f'-- left for FCC{quality_msg})')
    return clusters


def cluster_by_pattern_joint(
    all_readmes: dict, task_subs: dict[int, list[dict]],
    criterion: str, min_cluster_size: int = 2,
    use_quality_tiers: bool = False,
) -> dict:
    """Joint clustering: group students by ALL their patterns for a criterion.

    Unlike calling cluster_by_pattern 3 times separately, this returns all
    clusters in one pass and identifies students who have BOTH good and bad
    patterns (mixed_students).

    Args:
        all_readmes, task_subs, criterion, min_cluster_size: Same as cluster_by_pattern.
        use_quality_tiers: Include quality tier in grouping.
    ...
    Returns:
        {
            ...
            'good_clusters': [...],
            'mistake_clusters': [...],
            'mixed_students': set of student IDs that have both good and bad patterns,
            'all_bad_students': set of student IDs with any bad pattern,
            'all_good_students': set of student IDs with any good pattern,
        }
    """
    bad_clusters = cluster_by_pattern(
        all_readmes, task_subs, criterion, 'bad', min_cluster_size, use_quality_tiers)
    good_clusters = cluster_by_pattern(
        all_readmes, task_subs, criterion, 'good', min_cluster_size, use_quality_tiers)
    mistake_clusters = cluster_by_pattern(
        all_readmes, task_subs, criterion, 'mistake', min_cluster_size, use_quality_tiers)

    # Track which students have each type of pattern
    all_bad = set()
    for c in bad_clusters:
        all_bad.update(c['students'])
    for c in mistake_clusters:
        all_bad.update(c['students'])

    all_good = set()
    for c in good_clusters:
        all_good.update(c['students'])

    mixed = all_bad & all_good

    return {
        'bad_clusters': bad_clusters,
        'good_clusters': good_clusters,
        'mistake_clusters': mistake_clusters,
        'mixed_students': mixed,
        'all_bad_students': all_bad,
        'all_good_students': all_good,
    }
