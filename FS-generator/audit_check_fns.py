...
─────────────────────────────────────────────────────────
NO_FUNC_SCOPE           ✅ YES                Add "MUST filter by FunctionDef name" rule
MATCHES_TEMPLATE        ✅ YES (mostly)       Add "MUST NOT match template functions" rule
                                              + list template function names in prompt
MATCHES_REFERENCE       ⚠️  PARTIALLY         Prompt can help but some patterns are
                                              genuinely ambiguous (INSERT OR IGNORE is
                                              correct in reference, "bad" per README)
MATCHES_STUB            ✅ YES                Add "MUST NOT match pass-only stubs" rule
TYPE_A (NOT IN)         ✅ YES                Already prohibited in prompt; strengthen
''')
...
print(f'\nConclusion: {"❌ NOT 100% fixable" if ambiguous > 0 else "✅ 100% fixable by prompt"}')
if ambiguous > 0:
