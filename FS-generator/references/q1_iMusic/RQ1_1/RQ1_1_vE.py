    ...
    try:
        with open(playlist_tracks_file, 'r', newline='') as f:
            ...
            for row in reader:
                ...
                track_id = int(row[1])
                # Validate existence using SELECT EXISTS
                cursor.execute('SELECT EXISTS(SELECT 1 FROM Playlist WHERE PlaylistId = ?)', (playlist_id,))
                playlist_exists = cursor.fetchone()[0] == 1
                cursor.execute('SELECT EXISTS(SELECT 1 FROM Track WHERE TrackId = ?)', (track_id,))
