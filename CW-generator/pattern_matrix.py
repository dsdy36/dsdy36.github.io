...
def _ai_generate_variants(criterion_id: str, ptype: str, index: int,
    ...
    if not DEEPSEEK_API_KEY:
        ...
        if ptype == 'good':
            return [{'id': 'A', 'instruction': f'Implement: {description}',
                     'check_regex': r'.*', 'difficulty': 'basic'}]
        else:
            return [{'id': 'A', 'instruction': f'Implement incorrectly: {description}',
                     'check_regex': r'.*', 'difficulty': 'basic'}]
...
def _define_variants(criterion_id: str, ptype: str, index: int, description: str) -> list[dict]:
    ...
    if 'not closing' in desc_lower or 'resource leak' in desc_lower:
        return [
            {'id': 'A', 'instruction': 'Open sqlite3 connection with conn = sqlite3.connect(...) and perform cursor operations, but the conn.close() line is entirely absent from the function',
    ...
    if 'inserting without validation' in desc_lower:
        return [
            {'id': 'A', 'instruction': 'For each row parsed from the TSV, execute an INSERT INTO PlaylistTrack (PlaylistId, TrackId) VALUES (?, ?) statement without first running any SELECT query on the Playlist or Track tables.',
    ...
    if 'inserting duplicate' in desc_lower:
        return [
            {'id': 'A', 'instruction': 'For each row from the TSV, execute INSERT INTO PlaylistTrack (PlaylistId, TrackId) VALUES (?, ?) without any prior SELECT COUNT, INSERT OR IGNORE, or NOT IN check on the PlaylistTrack table.',
             'check_regex': r'INSERT\s+INTO\s+PlaylistTrack', 'difficulty': 'basic'},
        ]
    ...
    if 'not including' in desc_lower and 'all' in desc_lower:
        return [
            {'id': 'A', 'instruction': 'Query the Genre table with SELECT GenreId, Name FROM Genre ORDER BY Name ASC. Store the results in a list of dictionaries. Return that list directly without adding any extra dictionary at the beginning.',
    ...
    if 'not handling null' in desc_lower or 'returns none' in desc_lower:
        return [
            {'id': 'A', 'instruction': 'Use SUM(t.Milliseconds) and SUM(t.UnitPrice) directly in the SELECT clause without wrapping them in COALESCE() or IFNULL(), so the result shows NULL for playlists with no tracks.',
    ...
    if 'directly interpolating' in desc_lower:
        return [
            {'id': 'A', 'instruction': 'Build the ORDER BY clause using an f-string: query += f" ORDER BY {sort_column} {sort_order}". Do not add any validation of sort_column before using it.',
             'check_regex': r'f["\'].*ORDER\s+BY\s*\{', 'difficulty': 'basic'},
        ]
    if 'not validating' in desc_lower:
        return [
            {'id': 'A', 'instruction': 'Pass sort_by and sort_order directly into the SQL query string without any validation or sanitization.',
    ...
    if 'not validating genre' in desc_lower:
        return [
            {'id': 'A', 'instruction': 'Read GenreId from request.form and pass it directly to get_statistics() without first querying the Genre table to check if the genre exists.',
    ...
    if 'incorrect sort' in desc_lower:
        return [
            {'id': 'A', 'instruction': 'Execute the query SELECT PlaylistId, Name FROM Playlist without any ORDER BY clause, so the playlists appear in the order the database returns them.',
